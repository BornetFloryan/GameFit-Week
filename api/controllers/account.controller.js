...
exports.getCustomersAccounts = async (req, res) => {
    try {
        ...
        console.error(error);
        return res.state(500).send("Erreur lors de la récupération des comptes clients");
    }
}
...
exports.loginUser = async (req, res) => {
    try {
        ...
        if (data) {
            ...
            return res.state(404).send("Login/pass incorrect");
        }
    } catch (error) {
        console.error(error);
        return res.state(500).send("Erreur lors de la connexion de l'utilisateur");
    }
}
...
exports.addCustomerAccount = async (req, res) => {
    try {
        ...
        console.error(error);
        return res.state(500).send("Erreur lors de l'ajout du compte client");
    }
}
...
exports.modifyCustomerAccount = async (req, res) => {
    try {
        ...
        console.error(error);
        return res.state(500).send("Erreur lors de la modification du compte client");
    }
}